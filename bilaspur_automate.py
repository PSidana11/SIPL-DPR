...
# Configs
save_path = r"C:\Users\E01412\Desktop\REPORTS\DAILY\bilapsur"  # Update the path if needed
sender_name = "SIPL Bilaspur"  # <-- Use sender *name* here (not email)
target_script =[ "bilaspur.py", "bilaspur_finance.py"]  # Script to run after download
        ...
        if actual_sender.strip().lower() == sender_name.strip().lower() and received_date == today:
            for attachment in message.Attachments:
                if attachment.FileName.endswith(".xlsx"):
                    file_name = f"bilaspur_{today_str}.xlsx"
